<?php
function generateNomorHalaman($total_data, $offset, $limit, $cari=null)
{
    $hasil = "";
    $hasil .= "<p style='display: inline; margin-top: 20px;'>";

    $query = get_query($cari);
...
function get_query($cari = null) {
    ...
    if (!is_null($cari)) {
        ...
        if (!empty($faculty_id)) {
            $query .= "&cari_fakultas=" . $faculty_id; 
        }
    }
}
